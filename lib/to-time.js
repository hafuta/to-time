(function (global, factory) {
	...
	(global = typeof globalThis !== 'undefined' ? globalThis : global || self, global.toTime = factory());
	...
	var TimeFrame_1;
	var hasRequiredTimeFrame;

	function requireTimeFrame () {
		if (hasRequiredTimeFrame) return TimeFrame_1;
		hasRequiredTimeFrame = 1;

		const BigNumber = requireBignumber();
	...
	var ToTime;
	var hasRequiredToTime;

	function requireToTime () {
		if (hasRequiredToTime) return ToTime;
		hasRequiredToTime = 1;

		const BigNumber = requireBignumber();

		const TimeFrame = requireTimeFrame();
		const constants = requireConstants();
